/**
 * Lambda that deletes a smooch facebook integration
 * */
...
exports.handler = async (event) => {
  ...
  log.info('delete-facebook-integration was called', { ...logContext, params, SMOOCH_API_URL });
  ...
  try {
    ...
    const errMsg = 'An Error has occurred trying to delete an facebook integration';
  ...
  log.info('user deleted a smooch facebook-integration', {
    userId: identity['user-id'],
    tenantId,
    smoochIntegrationId: integrationId,
    audit: true,
  });
  log.info('delete-facebook-integration complete', logContext);

  return {
    status: 200,
    body: { message: `The facebook integration for tenant ${tenantId} and integrationId ${integrationId} has been deleted successfully`, deleted: true },
  };
};
